...
from app.database import engine, get_db, Base   # <- добавить app.
...
app = FastAPI(
    ...
    description="API для управления рецептами. Позволяет просматривать список рецептов, "
                "получать детальную информацию (с автоувеличением счётчика просмотров) "
...
async def recipe_detail(recipe_id: int, db: AsyncSession = Depends(get_db)):
    ...
    """
    # Проверяем существование перед инкрементом
    recipe = await crud.get_recipe_by_id(db, recipe_id)
    if not recipe:
        raise HTTPException(status_code=404, detail="Рецепт не найден")

    # Увеличиваем просмотры и получаем обновлённый объект
